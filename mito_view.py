...
if os.path.join(ROOT, "mito_client") not in sys.path:
    sys.path.insert(0, os.path.join(ROOT, "mito_client"))
if os.path.join(ROOT, "re") not in sys.path:
...
# ENGINE del binario replicado en Python (re/mito_engine.py): clases de
# entidades, fabrica FUN_14073b220, paletas reales, radio, fisica updateMouse.
from mito_engine import (CELL_RGB, FOOD_RGB, TEAM_RGB, radio_from_masa,
                         build_entity, ET_FOOD, ET_PLAYER, ET_VIRUS, ET_COIN,
                         ET_FLAG_BASE)
...
class MitoState:
    def __init__(self):
        ...
        self.autorespawn = False     # OFF por defecto: al morir -> lobby y
                                     # respawn manual con Enter/Respawn. ON:
                                     # el bot confirma el respawn solo.
...
def draw_gradient(surf):
    """Fondo oscuro profundo con gradiente sutil azulado.
    Se dibuja UNA sola vez en una Surface cacheada y se blitea en cada
    frame (H lineas por frame -> 1 blit)."""
    ...
    if _gradient_cache is None or _gradient_cache.get_size() != surf.get_size():
        ...
        ch = _gradient_cache.get_height()
        for y in range(ch):
            t = y / ch
            pygame.draw.line(_gradient_cache, lerp_color(BG_TOP, BG_BOT, t),
                             (0, y), (_gradient_cache.get_width(), y))
...
if __name__ == "__main__":
    ...
    pem = args.pem or os.path.join(ROOT, "embedded_rsa_private_14.pem")
    with STATE.lock:
